...
function hash(value, ancestors = [], update) {
    let circle_size = ~ancestors.indexOf(value);
    if (circle_size) {
        return update(circle_size);
    }
    ...
    if (deep_types.indexOf(type) !== -1) {
        ...
        Object.keys(value).sort().forEach(property => {
            hash(value[property], ancestors, v => target[property] = v);
        })
        update(index_and_store(type, proxy));
    } else if (shallow_types.indexOf(type) !== -1) {
        update(index_and_store(type, value));
    } else {
